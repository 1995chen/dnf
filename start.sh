...
docker run -e PUBLIC_IP=x.x.x.x -e DNF_DB_ROOT_PASSWORD=88888888 -e GM_ACCOUNT=gm_user -e GM_PASSWORD=gm_pass -v /data/log:/home/neople/game/log -v /data/mysql:/var/lib/mysql -v /data/data:/data --privileged=true --memory=8g --oom-kill-disable --shm-size=8g 1995chen/dnf:stable /bin/bash /home/template/init/init.sh
