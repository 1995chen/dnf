...
if [ "$SERVER_TYPE" = "GAME" ] || [ "$SERVER_TYPE" = "ALL" ]; then
  ...
  rm -rf /etc/supervisor/conf.d/tss.conf
  cp /etc/supervisor/conf.d/tss.conf.template /etc/supervisor/conf.d/tss.conf
  # 重新生成game配置文件[这里要重置下]
  ...
  if [ -n "$OPEN_CHANNEL" ]; then
    ...
    for num in $numbers; do
      if [[ $num -eq 1 || $num -eq 6 || $num -eq 7 || ($num -ge 11 && $num -le 39) || ($num -ge 52 && $num -le 56) ]];then
        ...
        echo "stdout_logfile=/dev/fd/1" >> /etc/supervisor/conf.d/game.conf
        echo "redirect_stderr=true" >> /etc/supervisor/conf.d/game.conf
        echo "stdout_logfile_maxbytes=0" >> /etc/supervisor/conf.d/game.conf
        echo "stderr_logfile_maxbytes=0" >> /etc/supervisor/conf.d/game.conf
        continue
      fi
  ...
  # 拷贝tss脚本
  if [ ! -f "/data/run/start_gunnersvr.sh" ];then
    cp /home/template/init/run/start_gunnersvr.sh /data/run/
    echo "init start_gunnersvr.sh success"
  else
    echo "start_gunnersvr.sh have already inited, do nothing!"
  fi

  if [ ! -f "/data/run/start_zergsvr_secagent.sh" ];then
    cp /home/template/init/run/start_zergsvr_secagent.sh /data/run/
    echo "init start_zergsvr_secagent.sh success"
  else
    echo "start_zergsvr_secagent.sh have already inited, do nothing!"
  fi

  if [ ! -f "/data/run/start_zergsvr.sh" ];then
    cp /home/template/init/run/start_zergsvr.sh /data/run/
    echo "init start_zergsvr.sh success"
  else
    echo "start_zergsvr.sh have already inited, do nothing!"
  fi
